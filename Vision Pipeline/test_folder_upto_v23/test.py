...
try:
    while True:
        ...
        if center:
            ...
            cv2.putText(frame, f"Estimated Distance from Depth Map: {distance:.2f} m", (10, 150), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
        ...
        # Step 10 Single Camera estimation methods
        # Inside the main loop
        if bounding_box:
            _, _, box_width, _ = bounding_box
            distance = distance_estimator.estimate_distance(box_width)
            if distance:
                cv2.putText(frame, f"Estimated Distance from Classical CV: {distance:.2f} m", (10, 180), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
